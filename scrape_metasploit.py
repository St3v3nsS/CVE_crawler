...
class MetasploitParser(Scraper):
    ...
    def parse_infos(self):
        ...
        try:
            ...
            lista = re.findall(r'\bdef initialize(?:\(\s*i|\s*super|\(\s*.*?\s*\)?\s*super)(.*?)(?:\bregister_options\b|\bend\b)\s*(?:def|[A-Za-z_0-9]+\s*=)', self.exploit, re.DOTALL)[0]    # Get the main initialize function
    ...
    def parse_url(self):
        ...
        file.write(self.filename + '\n')
        register_options = re.findall('register_options\((.*?)],?(?:\s*self\.class)?\)?\s*(?:end)', self.exploit, re.DOTALL)
        if register_options:
        ...
        if normalize:
            ...
            URIs.extend(self.construct_url(['/' + '/'.join(s.strip('/').replace("'", '').replace(',', '').replace('"', '').replace(' ', '') for s in normaliz[i].split(',') if 'uri' not in s and 'nil' not in s) for i in range(len(normaliz))]))
        file.write(str(URIs) + '\n')
    ...
    def find_desc(self, description):
        ...
        for i, word in enumerate(description):
            ...
            if (count == 0 and found and word == self.revert(delimiter)):    # If there's a matching paranthesis and a odd number of them
                return (start, i+1, is_q)
            elif (description[i-1] == self.revert(delimiter) and word == ',' and "".join(description[i+1:].split())[0] in "\'\""):   # There can be some anomalies and try to find the coresponding paranthesis in construction like ")'Author"
                return (start, i, is_q)
