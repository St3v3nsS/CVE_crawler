...
elFinder.prototype.commands.download = function () {
      ...
      if (mixed) {
        ...
        hashes = jQuery.grep(hashes, function (h) {
          var f = fm.file(h),
            res =
              !f ||
              (!czipdl && f.mime === "directory") ||
              !fm.isCommandEnabled(mixedCmd, h)
                ? false
                : true;
          if (f && inExec && !res) {
  ...
  this.getstate = function (select) {
    ...
    return cnt &&
      (zipOn || (cnt <= maxReq && ((!fm.UA.IE && !fm.UA.Mobile) || cnt == 1)))
      ? 0
      : -1;
  };
      ...
      getExtra = function (file) {
        ...
        return {
          ...
          node: jQuery("<a></a>")
            ...
            .text(file.name)
            .on(
              "mousedown click touchstart touchmove touchend contextmenu",
              function (e) {
                e.stopPropagation();
              }
            )
            ...
            node: jQuery("<a></a>")
              ...
              .on("click touchstart", function (e) {
                ...
                parent
                  ...
                  .addClass("elfinder-button-icon-spinner");
                fm.request({
  ...
  this.exec = function (select) {
                ...
                zipdlFn = function (url) {
                  ...
                  if (linkdl) {
                    dllink
                      .attr("target", "_blank")
                      .append(
                        '<span class="elfinder-button-icon elfinder-button-icon-download"></span>' +
                          fm.escape(dlName)
                      );
    ...
    if (
      zipOn &&
      (fileCnt !== files.length || fileCnt >= (this.options.minFilesZipdl || 1))
    ) {
      link.remove();
      ...
      getUrlDfrd = jQuery.Deferred().done(function (urls) {
        for (i = 0; i < urls.length; i++) {
          ...
          if (
            dlntf &&
            url.substr(0, fm.options.url.length) === fm.options.url
          ) {
          ...
          if (html5dl) {
            ...
            if (fm.UA.Mobile) {
              ...
              iframes +=
                '<iframe class="downloader" id="downloader-' +
                files[i].hash +
