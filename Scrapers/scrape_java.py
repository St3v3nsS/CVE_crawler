    ...
    def parse_infos(self):
        ...
        try:
            ...
            if source_at_begin:
                ...
                if ('###' not in source_at_begin[2] or 'import' not in source_at_begin[2]):
                    description.extend([source_at_begin[2]])
                if len(source_at_begin[1]) > 2 and 'import' not in source_at_begin[3] or 'package' not in source_at_begin[3]\
                        or '//' not in source_at_begin[3] or '/*' not in source_at_begin[3]:
            ...
            for comment in comments:
                ...
                vversion.extend(re.findall('[vV]ersions?\s*:?\s*(.*)', comment))
