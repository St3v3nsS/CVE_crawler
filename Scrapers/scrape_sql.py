...
class SQLScraper(Scraper):
    ...
    def parse_infos(self):
        ...
        try:
            ...
            refs.extend(re.findall('See also:\s*(.*?)\n--\n', self.exploit, flags=re.S))
            vversion.extend(re.findall('Vulnerable platforms:\s*(.*?)\n--\n', self.exploit, flags=re.S))
            targets.extend(re.findall('Tested on\s*(.*?)\n--\n', self.exploit, flags=re.S))

            id_in_exploit = re.findall('\$Id.*?\n--\n(.*?)\n--\n(.*?)\n--\n(.*?)\n--\n', self.exploit, flags=re.S)
            if id_in_exploit:
                id_in_exploit = id_in_exploit[0]
                name.extend([id_in_exploit[0]])
                if id_in_exploit[1]:
                    description.extend([id_in_exploit[1]])
                if id_in_exploit[2] and 'create or replace' not in id_in_exploit[2]:
                    targets.extend([id_in_exploit[2]])
